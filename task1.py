...
train_images = load_images('data/train/Task1')
train_queries = load_queries('data/train/Task1')
lane_images = load_images('data/train/Task1/full-configuration-templates')
lane_pins = load_lane_pins('data/train/Task1/full-configuration-templates')
...
for image, query in zip(train_images, train_queries):
    ...
    # see_structure_first = get_sorted_pins_position(model, lane_images[lane - 1], classes=[39,75], conf=0.02, verbose=0)
    # full_pins_pos = see_structure_first
    ...
    if i<10:
        file_path = os.path.join(f"{os.getcwd()}\\results\\Task1", f"0{i}_predicted.txt")
    else:
        file_path = os.path.join(f"{os.getcwd()}\\results\\Task1", f"{i}_predicted.txt")

    ensure_directory_exists(os.path.dirname(file_path))

    with open(file_path, 'w') as file:
        num_predicted_pins = query[0]
...
predicted_truths = load_results("results/Task1")
